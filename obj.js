...
//метод shift - удаляет элемент в начале , и освобождает очередь 2 элементу, который соответственно после удаления встанет на месо первого и будет первым в массиве 
yrt.shift();
console.log(yrt);
...
//если хотим копировать с конца то как просто указываем значения со знаком - 


//что бы копировать все массив указывваем slise без значений 
...
////метод filter делает можно сказать тоже самое что и find только фильтр ищет все значения в каком то смысле среди массивов
...
const newSpl = spl.split(',',3);//создание массива и его поределенние по разделителям , а тк же указание сколько переменных попадет в масссив 
